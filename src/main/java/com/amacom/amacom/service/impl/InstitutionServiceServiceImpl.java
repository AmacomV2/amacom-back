...
public class InstitutionServiceServiceImpl implements IInstitutionServiceService {
    ...
    @Override
    public Page<InstitutionService> findInstitutionService(UUID idInstitution, UUID idService, String query,
            Pageable pageable) {
        ...
        if (pageable.getSort().isUnsorted()) {
            Pageable pageableDefault = PageRequest.of(pageable.getPageNumber(), pageable.getPageSize(),
                    Sort.by("institution.name").ascending()
                            .and(Sort.by("institution.createdAt").descending()));
            institutionServicePage = this.institutionServiceRepository.findInstitutionService(idInstitution, idService,
    ...
    public InstitutionService update(InstitutionService institutionService) {
        var institutionServiceBD = this.institutionServiceRepository.findById(institutionService.getId())
    ...
    public void deleteById(UUID id) {
        var institutionServiceBD = this.institutionServiceRepository.findById(id)
                .orElseThrow(DataNotFoundException::new);
